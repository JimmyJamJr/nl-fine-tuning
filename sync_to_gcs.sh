...
# Default GCS layout: nl-runs/<RUN_NAME>/ keeps every job self-contained under one
# umbrella prefix. RUN_NAME is basename(RUN_DIR) which already encodes a UTC
# timestamp from the launcher (e.g. job_local_20260427_174721_qwen06b_nocurr120),
# so the destination directory is well-timestamped without further work.
# Override DEST_PREFIX explicitly to slot into a per-experiment family directory,
# e.g. DEST_PREFIX=qwen06b-nocurr-L120/${RUN_NAME}.
DEST_PREFIX="${DEST_PREFIX:-nl-runs/${RUN_NAME}}"
