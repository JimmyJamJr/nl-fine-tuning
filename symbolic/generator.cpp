...
bool generate_graph_with_lookahead(array<node>& vertices, node*& start, node*& end, unsigned int num_vertices, unsigned int max_num_parents, unsigned int max_vertex_id, unsigned int lookahead, unsigned int num_paths, unsigned int max_prefix_vertices)
{
	...
	if (lookahead == 0) {
		...
		for (unsigned int j = 0; j < num_paths - 1; j++) {
			...
			// compute how many extra steps we are *allowed* to take
            int branch_budget = (int)num_vertices
                              - (int)index
                              - (int)(num_paths - j - 1) * (int)lookahead
                              + 2;

            if (branch_budget < 0)
                branch_budget = 0;

            unsigned int max_extra = std::min(2u, (unsigned int)branch_budget);

            unsigned int other_branch_length = lookahead;
            if (max_extra > 0) {
                // safe: randrange(n) with n>0 returns 0..n-1
                other_branch_length += randrange(max_extra);
            }
...
//			unsigned int other_branch_length = lookahead + randrange(std::min(2u, num_vertices - index - (num_paths - j - 1) * lookahead + 2));
			for (unsigned int i = 1; i < other_branch_length; i++) {
				...
				if (index >= num_vertices) {
                    // safety: bail out if we somehow ran out of allocated vertices
                    break;
                }
			}
		}
	...
	for (unsigned int i = index; i < num_vertices; i++) {
		...
		if (index == 0 || num_children == 0) {
            // can't actually add children that point backward yet,
            // or we just don't want any.
            num_children = 0;
        }
        else {
            for (unsigned int j = 0; j < num_children; j++) {
                if (total_probability <= 0.0f || probabilities.length == 0) {
                    num_children = j;
                    break;
                }
		...
		if (total_probability != 0.0f && index > descendants.length && num_parents > 0) {
...
py::tuple generate_training_set(const unsigned int max_input_size, const uint64_t dataset_size, int max_lookahead, const unsigned int max_edges_input, const py::object& reserved_inputs, const int distance_from_start, const int max_prefix_vertices, const bool quiet=false, float alpha=1.0)
{
    ...
    // Cap lookahead by tokens and by the alpha-scaled edge budget
    const unsigned int TOKEN_EDGE_CAP = (max_input_size - 5) / 3;
    unsigned int E_max = std::min(max_edges, TOKEN_EDGE_CAP);
    const unsigned int L_token = ((max_input_size - 5) / 3 - 1) / 2;  // floor((n-8)/6)
    const unsigned int L_alpha = E_max / 2;
    unsigned int L_user = (max_lookahead < 0) ? UINT_MAX : (unsigned int) max_lookahead; // needs <climits>
    unsigned int L_eff  = std::min(L_token, std::min(L_alpha, L_user));
    max_lookahead = (int)L_eff;
	...
	while (num_generated < dataset_size) {
		...
		while (true) {
			if (max_lookahead == -1) {
				// Scale num_vertices based on alpha
				unsigned int num_vertices = randrange(3, max_vertices);
				if (!generate_example(g, start, end, paths, num_vertices, 4, max_vertex_id, true, -1, 0, max_prefix_vertices == -1 ? max_input_size : max_prefix_vertices)) {
				...
				if (lookahead == 0) {
				    ...
				    // avoid unsigned underflow and division by zero weirdness
                    unsigned int safe_edges = (max_edges > 1 ? max_edges - 1 : 1);
					unsigned int max_num_paths = safe_edges / (unsigned int) lookahead;
                    if (max_num_paths < 2)
                        max_num_paths = 2;

                    unsigned int upper = max_num_paths + 1;
                    if (upper <= 2) upper = 3;
					num_paths = randrange(2, upper);
				}

				// Scale num_vertices based on alpha
				unsigned int num_vertices = std::min(std::min(lookahead * num_paths + 1 + randrange(0, 6), max_vertices), max_edges + 1);
				if (!generate_example(g, start, end, paths, num_vertices, 4, max_vertex_id, true, lookahead, num_paths, max_prefix_vertices == -1 ? max_input_size : max_prefix_vertices)) {
					for (node& n : g) core::free(n);
					for (array<node*>& a : paths) core::free(a);
					g.length = 0; paths.length = 0;
					continue;
		...
		for (const array<node*>& path : paths) {
			...
			for (unsigned int j = 1; j < path.length; j++) {
				...
				if (max_lookahead != -1 && lookahead_steps != lookahead)
					continue;
				if (lookahead_steps < max_input_size)
...
py::tuple generate_dfs_training_set(const unsigned int max_input_size, const uint64_t dataset_size, const py::object& reserved_inputs, const int requested_backtrack, const bool random_padding, const bool uniform, const bool quiet=false, float alpha=1.0)
{
	const unsigned int QUERY_PREFIX_TOKEN = (max_input_size-5) / 3 + 4;
	const unsigned int PADDING_TOKEN = (max_input_size-5) / 3 + 3;
	const unsigned int EDGE_PREFIX_TOKEN = (max_input_size-5) / 3 + 2;
	...
	// Apply alpha scaling to control graph complexity
	unsigned int longest_path_length = (unsigned int)(alpha * (max_input_size - 4) / 4);
	unsigned int max_edges = longest_path_length;  // For DFS, these are typically the same

	// Scale max_vertex_id based on alpha (similar to SI task)
	unsigned int max_vertex_id = max_edges + 1;  // Keep it simple like SI task
	...
	while (num_generated < dataset_size) {
		...
		while (true) {
			...
			if (requested_backtrack != -1)
				num_vertices = std::max((unsigned int) backtrack + 2, num_vertices);
			// Use similar approach to SI task for max_num_parents
			if (!generate_dfs_example(g, start, end, current_node_index, path, num_vertices, max(3, max_input_size / 24 + 1), max_vertex_id, backtrack, max_edges)) {
				for (node& n : g) core::free(n);
				g.length = 0; path.length = 0;
				continue;
			}
		...
		for (node& vertex : g)
			for (node* child : vertex.children)
				edges.add(make_pair(vertex.id, child->id));
		// Use scaled max_edges instead of longest_path_length
		...
		if (!quiet && num_generated > 0 && (num_generated % 1000 == 0 || num_generated >= dataset_size)) {
			printf("%d examples generated (alpha=%.2f, max_edges=%d).\n", num_generated, alpha, max_edges);
			fflush(stdout);
...
bool generate_si_example(array<node>& vertices, const node*& start, const node*& end, unsigned int& current_node_index, array<const node*>& path, unsigned int num_vertices, unsigned int max_num_parents, unsigned int max_vertex_id, unsigned int max_edges, unsigned int frontier_size, unsigned int branch_size)
{
	...
	// NEW CODE
	// Only extend path by 0-3 more edges instead of up to max_edges
	// for (unsigned int i = path.length / 2; i < min(path.length / 2 + 4, max_edges + 1); i++)
    // 	path_length_probabilities.add((float) i);
...
//			if (!generate_si_example(g, start, end, current_node_index, path, num_vertices, max_input_size / 24 + 1, (max_input_size - 2) / 6 + 2, max_edges, frontier_size, branch_size)) {
			...
			// OLD CODE
			unsigned int max_vertex_id = max_edges + 1;
			// unsigned int max_vertex_id = (max_input_size - 2) / 6 + 1;

			// 	NEW CODE
			// unsigned int max_vertex_id = (max_input_size - 2) / 6 + 2;

			// NEW CODE RELAXED
			// unsigned int max_vertex_id = (max_input_size - 5) / 3 - 1;

			if (!generate_si_example(g, start, end, current_node_index, path, num_vertices, max_input_size / 24 + 1, max_vertex_id, max_edges, frontier_size, branch_size)) {
				for (node& n : g) core::free(n);
				g.length = 0; path.length = 0;
				continue;
			}
