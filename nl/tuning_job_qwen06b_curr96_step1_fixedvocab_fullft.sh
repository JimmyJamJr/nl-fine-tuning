#!/bin/bash
#SBATCH -J nl_qwen06b_curr96in128_step1_fixedvocab_fullft
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:4
...
# Ablation: vocab pool fixed at the L=128 maximum (max_input_size=768 ⇒
# max_vertex_id pinned at 254). Curriculum trains stages L=1..96 only;
# training stops when L=96 clears OR cumulative compute hits 60K PFLOPs,
# whichever comes first.
#
# - --fixed_vocab: keeps vertex-ID pool at its un-alpha-scaled max (so vocab
#   does NOT scale with curriculum). Graph size (max_edges, max_vertices)
#   still scales with alpha.
# - max_input_size=768 sizes the vocab pool for L=128 (alpha=1.0 equivalent).
# - max_lookahead=96 + n_stages=96 means _is_final_stage fires at L=96, which
#   triggers control.should_training_stop in the curriculum callback.
# - --max_total_pflops 60000 caps compute regardless of curriculum progress.
#
# Uses tuning_nl.py (FA2; works without flash_attn_3).
...
# Task parameters
MAX_INPUT_SIZE=768                       # 6 * 128 — vocab pool sized for L=128
MAX_LOOKAHEAD=96                         # curriculum cap; _is_final_stage fires at L=96
MAX_FRONTIER_SIZE=12
MAX_BRANCH_SIZE=12
REQUESTED_BACKTRACK=3

# Compute budget cap (whichever fires first: L=96 complete OR this PFLOPs cap)
...
echo "Task: $TASK | Model: $MODEL_NAME | Max Input: $MAX_INPUT_SIZE | FIXED VOCAB ABLATION"
...
try:
    import generator
    # Verify generator supports fixed_vocab
    import inspect
    sig = generator.generate_training_set.__doc__ or ''
    if 'fixed_vocab' not in sig:
        print('[INFO] Compiled generator predates fixed_vocab; rebuilding...')
        raise ImportError
    print('[OK] C++ generator with fixed_vocab support present')
except Exception:
...
ARGS=(
    ...
    # *** ABLATION: vocab pinned at full pool ***
    --fixed_vocab

    # Compute cap (early-exit before L=96 if 60K PFLOPs hits first)
