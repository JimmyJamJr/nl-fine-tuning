...
echo "========================================"
echo "BATCH LIMIT -- find max batch_size before OOM (L=128)"
echo "$(date)"
echo "========================================"
...
# Test increasing batch_size at L=128
# 96 uses 41GB/80GB. Try 192, 288, 384, 480
# Format: "label batch_size base_lookahead lookahead_step"
CONFIGS=(
    "bs192_L128    192 128 0"
    "bs288_L128    288 128 0"
    "bs384_L128    384 128 0"
    "bs480_L128    480 128 0"
)
...
for cfg in "${CONFIGS[@]}"; do
    read -r LABEL BS BASE_L L_STEP <<< "$cfg"

    echo ""
    echo "========================================"
    echo "CONFIG: $LABEL"
    echo "  batch_size=$BS base_L=$BASE_L"
    echo "========================================"
    ...
    if [ $EXIT_CODE -ne 0 ]; then
        echo "[WARN] Config $LABEL failed with exit $EXIT_CODE (likely OOM)"
    fi
...
echo ""
echo "BATCH LIMIT BENCHMARK COMPLETE -- $(date)"
